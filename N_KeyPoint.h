...
#ifndef __N_parallelsurf_keypoint_h
#define __N_parallelsurf_keypoint_h
...
class N_KeyPoint
{
	public:
		N_KeyPoint();
		N_KeyPoint ( double x, double y, double s, double score, int trace );
...
inline N_KeyPoint::N_KeyPoint()
{

}

inline N_KeyPoint::N_KeyPoint ( double x, double y, double s, double score, int trace ) :
		_x ( x ), _y ( y ), _scale ( s ), _score ( score ), _trace ( trace ), _vec ( 0 )
{

}

inline bool operator < ( const N_KeyPoint & iA, const N_KeyPoint & iB )
{
	return ( iA._score < iB._score );
}

inline std::ostream& operator<< ( std::ostream &out, const N_KeyPoint &keyPoint )
{
