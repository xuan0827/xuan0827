...
N_KeyPointDescriptor::N_KeyPointDescriptor ( N_Image& iImage, bool iExtended ) :
...
void N_KeyPointDescriptor::makeDescriptor ( N_KeyPoint& ioKeyPoint ) const
{
	// create a descriptor context
	N_KeyPointDescriptorContext aCtx ( _subRegions, _vecLen, ioKeyPoint._ori );
	...
	// normalize
	N_Math::Normalize ( ioKeyPoint._vec );

}

void N_KeyPointDescriptor::assignOrientation ( N_KeyPoint& ioKeyPoint ) const
{
	unsigned int aRX = N_Math::Round ( ioKeyPoint._x );
	unsigned int aRY = N_Math::Round ( ioKeyPoint._y );
	int aStep = ( int ) ( ioKeyPoint._scale + 0.8 );

	N_WaveFilter aWaveFilter ( 2.0 * ioKeyPoint._scale + 1.6, _image );
	...
	while ( aWindowBegin < aRespVector.size() )
	{
		...
		if ( aWindowSize < PI / 3 )
		{
			...
			{
				aMax.magnitude = (float)aMagnitudeSum;
				aMax.orientation = (float)aOrientationSum;
			}
			aWindowEnd++;
			if ( aWindowEnd >= aRespVector.size() )
			{
				aWindowEnd = 0;
				aOriAdd += (float)2 * (float)PI;
			}
	...
	ioKeyPoint._used = false;
}



void N_KeyPointDescriptor::createDescriptor ( N_KeyPointDescriptorContext& iCtx, N_KeyPoint& ioKeyPoint ) const
{
	...
	int aIntX = N_Math::Round ( aX );
	int aIntY = N_Math::Round ( aY );
	int aIntS = N_Math::Round ( aS / 2.0 );
	if ( aIntS < 1 ) aIntS = 1;
	...
	int aIntRadius = N_Math::Round ( aRadius / aIntS );
	if ( aS < 1 ) aS = 1;
	...
	for ( int aYIt = -aIntRadius; aYIt <= aIntRadius; ++aYIt )
	{
		for ( int aXIt = -aIntRadius; aXIt <= aIntRadius; ++aXIt )
		{
			     ...
			     aVIdx >= -1.0 && aVIdx < _subRegions )
			{
				...
				N_WaveFilter aWaveFilter ( aStep, _image );
...
N_KeyPointDescriptorContext::N_KeyPointDescriptorContext (	int iSubRegions,
  int iVecLen, double iOrientation ) :
...
N_KeyPointDescriptorContext::~N_KeyPointDescriptorContext()
{
...
int N_KeyPointDescriptor::getDescriptorLength() const
{
